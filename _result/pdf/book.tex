...
When the process currently using the resources does not use it anymore, it signals it to the semaphore sending the message \textcode{signal}. 
...
\begin{center}
\includegraphics[width=0.6\textwidth]{/Users/ducasse/Workspace/FirstCircle/MyBooks/Bk-Writing/PharoBooks2/Booklet-ConcurrentProgramming/_result/pdf/Chapters/figures/Semaphores4.pdf}\caption{P0 has finished to use the resources: it signals it to the semaphore. The semaphore resumes the first pending process. \label{Sema4}}\end{center}
\end{figure}
...
\begin{center}
\includegraphics[width=0.6\textwidth]{/Users/ducasse/Workspace/FirstCircle/MyBooks/Bk-Writing/PharoBooks2/Booklet-ConcurrentProgramming/_result/pdf/Chapters/figures/Semaphores5.pdf}\caption{The resumed process, P1,  is added to the scheduled list of process of the ProcessScheduler.\label{Sema5}}\end{center}
\end{figure}
...
Unlike the \textcode{ProcessorScheduler}, a semaphore does not pay attention to the priority of a process, it dequeues processes in the order in which they waited on the semaphore.
...
A semaphore can be rearmed: it can be signalled (receives \textcode{signal} messages) before receiving \textcode{wait} messages. 
...
It will not block the process sending a \textcode{wait} message if it has got  \textcode{signal} messages that did not led to scheduling a waiting process.
\subsection{Example }
...
This example illustrates that signalling a semaphore does not have to be done after a \textcode{wait}.
...
We can ask a semaphore whether if it is prearmed using the message \textcode{isSignaled}. 
...
For this the class \textcode{Semaphore} offers the message \textcode{critical: aBlock}.
It evaluates aBlock only if the receiver is not currently in the process of running the \textcode{critical:} message. 
If the receiver is, evaluate aBlock after the other \textcode{critical:} message is finished.
To use a critical, first the semaphore should be prearmed using the class creation message \textcode{forMutualExclusion}
\subsection{Example.}
\begin{displaycode}{plain}
...
If the number of waiting process on a semaphore is smaller than the number allowed to wait, sending a \textcode{wait} message is not blocking and the process can continue its operations. 
...
Here is the implementation of \textcode{signal} and \textcode{wait} in Pharo.

The \textcode{signal} method shows that If there is no waiting process, the excess signal is increased, else when there are waiting processes, the first one is scheduled.
...
The \textcode{wait} method shows that when a semaphore has some signals on excess, waiting is not blocking, it just decreases the number of signals on excess. 
...
An instance of the class \textcode{Mutex} will make sure that only one thread of control can be executed simultaneously on a given portion of code using the message \textcode{critical:}.
...
As we saw, using \textcode{wait} and \textcode{signal} we can  make sure that two programs running in separate threads can be executed one after the other in order.

The following example is freely inspired from \symbol{34}The little book of semaphores book.
...
We can enforce such order by using \textcode{signal} and \textcode{wait} as following
...
A Mutex is a semaphore with a little more information: the current process running held in the \textcode{owner} instance variable. 
...
The \textcode{initialize} method makes sure that the semaphore is prearmed for mutual exclusion.
...
The key method is the method \textcode{critical:}. It checks if the owner of the mutex is the current thread.
